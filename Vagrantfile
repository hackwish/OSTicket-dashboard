  ...
  nodos.each do |name, options|
      config.vm.define name do |node|
       ...
       node.vm.synced_folder "app", "/var/www/html/#{options[:host]}"
        ...
        node.vm.provision :shell, :inline => "cp /vagrant/010-osdash.bydefault.dev.conf /etc/apache2/sites-available/"

        node.vm.provision :shell, :inline => "echo '127.0.1.1 bydefault.dev osdash.bydefault.dev' >> /etc/hosts"
        node.vm.provision :shell, :inline => "echo '#{options[:ip]} bydefault.dev osdash.bydefault.dev' >> /etc/hosts"
        
        node.vm.provision :shell, :inline => "a2ensite 010-osdash.bydefault.dev && service apache2 restart"
        node.vm.provision :shell, :inline => "mysql --defaults-extra-file=/vagrant/config.cnf -e 'drop database if exists osticket;'"
        node.vm.provision :shell, :inline => "mysql --defaults-extra-file=/vagrant/config.cnf -e 'create database osticket;'"
        node.vm.provision :shell, :inline => "mysql --defaults-extra-file=/vagrant/config.cnf osticket < /vagrant/osticket.sql"
